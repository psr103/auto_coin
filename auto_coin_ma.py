        ...
        if start_time < now < end_time - datetime.timedelta(seconds=10):
            ...
            if target_price < current_price and ma15 < current_price:
                ...
                if krw > 5000:
                    ...
                    krw = get_balance("KRW")
                    btc = get_balance("BTC")
                    post_message(myToken,"#autocoin","매수 진행")
                    post_message(myToken,"#autocoin","원화 잔액: %s")%krw
                    post_message(myToken,"#autocoin","코인 잔액: %s")%btc
            ...
            if btc > 0.00008:
                ...
                krw = get_balance("KRW")
                btc = get_balance("BTC")
                post_message(myToken,"#autocoin","전량 매도 진행")
                post_message(myToken,"#autocoin","원화 잔액: %s")%krw
                post_message(myToken,"#autocoin","코인 잔액: %s")%btc
        time.sleep(1)
